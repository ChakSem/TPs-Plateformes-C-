 ...
 */
int parserJson::saveData(Data& data) {
 ...
 * Sortie : Rien
 */
int parserJson::updateData(Data& data) {
    try {
        QFile fileUser(FILEPATHUSER);
        if (!fileUser.open(QIODevice::ReadOnly)) {
            /* Affiche un message d'erreur en cas d'échec d'ouverture du fichier .json */
 ...
 */
void parserJson::setPassword(QString id, QString password) {
    try {
 ...
 * Sortie : std::string
 */
QString parserJson::getPassword(QString id) {
    try {
